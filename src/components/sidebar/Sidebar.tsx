...
interface Props {
  ...
  typFilter: SchoolTypKey | null;
  onTypFilter: (t: SchoolTypKey | null) => void;
    ...
    <aside className="sidebar">
      <div className="sb-header">
        ...
        <div className="sb-sub">
          {allSchulen.length} Einrichtungen · {Object.keys(SCHULTYPEN).length} Schularten
        </div>
      </div>
        ...
        {Object.values(SCHULTYPEN).map((t) => {
          ...
          return (
            <button
              ...
              onClick={() => onTypFilter(active ? null : (t.key as SchoolTypKey))}
            >
              <span className="dot" style={{ background: t.color }} />
              {t.short}
              <span style={{ opacity: 0.6, fontFamily: 'var(--font-mono)', fontSize: 11 }}>{count}</span>
            </button>
          );
        })}
      </div>
      ...
      <div className="sb-footer">
        <svg viewBox="0 0 16 16" width="12" height="12" fill="none" stroke="currentColor" strokeWidth="1.4">
          ...
          <path d="M8 5 V9 M8 11 V11.5" strokeLinecap="round" />
        </svg>
